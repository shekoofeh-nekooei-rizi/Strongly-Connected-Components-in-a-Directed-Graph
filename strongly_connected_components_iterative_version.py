...
In this implementation I used iterative approach rather than recursive approach.
In other words, for the depth first search, I used stacks instead of recursivly call dfs function.
...
Vertices are represented by integers (starting from 0 or 1).
...
def strongly_connected_components_iterative(number_of_vertices, list_of_edges):
    # Here number_of_vertices is indeed the largest number among vertices. Thus, if vertices start from 0 (rather than 1) then the total number of vertices in the graph is number_of_vertices equls + 1
    start_vertex = 1
